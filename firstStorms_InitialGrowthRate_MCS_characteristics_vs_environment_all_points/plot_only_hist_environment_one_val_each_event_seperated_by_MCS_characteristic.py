...
Plots distributions of environmental variables seperated by MCS characteristics derived all points data but for a calculated statistic (e.g. spread) over the area at MCS initation times.
...
MCS_growth_stage_time_length_filtered_spreadByEvent = np.array([arr[0,0] for arr in MCS_growth_stage_time_length_filtered_all_points_byEvent])
...
MUCAPE_spreadByEvent = np.array([np.nanmax(arr)-np.nanmin(arr) for arr in MCS_MUCAPE_all_points_byEvent])
...
MCS_ccs_area_growth_rate_filtered_spreadByEvent = MCS_ccs_area_growth_filtered_spreadByEvent/MCS_growth_stage_time_length_filtered_spreadByEvent

# Filter events
if events_removed == True:

    events_bool = np.full(len(bulk_shear_0_3km_spreadByEvent), True)
...
MCS_prop_area_SALLJ_spreadByEvent = np.array(MCS_prop_area_SALLJ_spreadByEvent)[events_bool]
MCS_duration_filtered_spreadByEvent = np.array(MCS_duration_filtered_spreadByEvent)[events_bool]
median_SALLJ_max_wind_spreadByEvent = np.array(median_SALLJ_max_wind_spreadByEvent)[events_bool]
median_SALLJ_height_spreadByEvent = np.array(median_SALLJ_height_spreadByEvent)[events_bool]
MCS_ccs_area_growth_filtered_spreadByEvent = np.array(MCS_ccs_area_growth_filtered_spreadByEvent)[events_bool]
MCS_growth_stage_time_length_filtered_spreadByEvent = np.array(MCS_growth_stage_time_length_filtered_spreadByEvent)[events_bool]
MCS_ccs_area_growth_rate_filtered_spreadByEvent = np.array(MCS_ccs_area_growth_rate_filtered_spreadByEvent)[events_bool]
bulk_shear_0_3km_spreadByEvent = np.array(bulk_shear_0_3km_spreadByEvent)[events_bool]
bulk_shear_0_6km_spreadByEvent = np.array(bulk_shear_0_6km_spreadByEvent)[events_bool]
bulk_shear_2_6km_spreadByEvent = np.array(bulk_shear_2_6km_spreadByEvent)[events_bool]
q_850_spreadByEvent = np.array(q_850_spreadByEvent)[events_bool]
MUCAPE_spreadByEvent = np.array(MUCAPE_spreadByEvent)[events_bool]

# remove events where growth rate is nan (because MCS was found at time 0)

nan_indices = np.isnan(MCS_ccs_area_growth_rate_filtered_spreadByEvent)
...
MCS_ccs_area_growth_filtered_spreadByEvent = np.delete(MCS_ccs_area_growth_filtered_spreadByEvent, nan_indices)
MCS_growth_stage_time_length_filtered_spreadByEvent = np.delete(MCS_growth_stage_time_length_filtered_spreadByEvent, nan_indices)
MCS_ccs_area_growth_rate_filtered_spreadByEvent = np.delete(MCS_ccs_area_growth_rate_filtered_spreadByEvent, nan_indices)
bulk_shear_0_3km_spreadByEvent = np.delete(bulk_shear_0_3km_spreadByEvent, nan_indices)
bulk_shear_0_6km_spreadByEvent = np.delete(bulk_shear_0_6km_spreadByEvent, nan_indices)
bulk_shear_2_6km_spreadByEvent = np.delete(bulk_shear_2_6km_spreadByEvent, nan_indices)
q_850_spreadByEvent = np.delete(q_850_spreadByEvent, nan_indices)
MUCAPE_spreadByEvent = np.delete(MUCAPE_spreadByEvent, nan_indices)

# calculate median growth rate

median_MCS_ccs_area_growth_rate_filtered_spreadByEvent = np.nanmedian(MCS_ccs_area_growth_rate_filtered_spreadByEvent)
print(median_MCS_ccs_area_growth_rate_filtered_spreadByEvent)
...
var_seperation = MCS_ccs_area_growth_rate_filtered_spreadByEvent
var_seperation_name = 'ccs_area_growth_rate'
seperation_threshold = median_MCS_ccs_area_growth_rate_filtered_spreadByEvent
...
MCS_growth_stage_time_length_filtered_spreadByEvent_mask1 = MCS_growth_stage_time_length_filtered_spreadByEvent[mask1]
MCS_ccs_area_growth_rate_filtered_spreadByEvent_mask1 = MCS_ccs_area_growth_rate_filtered_spreadByEvent[mask1]
...
MUCAPE_spreadByEvent_mask1 = MUCAPE_spreadByEvent[mask1]
...
print('# rapid growth MCS', len(MCS_ccs_area_growth_rate_filtered_spreadByEvent_mask1))
...
MCS_growth_stage_time_length_filtered_spreadByEvent_mask2 = MCS_growth_stage_time_length_filtered_spreadByEvent[mask2]
MCS_ccs_area_growth_rate_filtered_spreadByEvent_mask2 = MCS_ccs_area_growth_rate_filtered_spreadByEvent[mask2]
...
MUCAPE_spreadByEvent_mask2 = MUCAPE_spreadByEvent[mask2]
...
print('# slow growth MCS', len(MCS_ccs_area_growth_rate_filtered_spreadByEvent_mask2))

############ Change variable plotting ############ 

data_all_growth = bulk_shear_0_3km_spreadByEvent
data_rapid_growth = bulk_shear_0_3km_spreadByEvent
data_slow_growth = bulk_shear_0_3km_spreadByEvent
variable_name = 'bulk_shear_0_3km_spreadByEvent' # q_850_spreadByEvent, bulk_shear_0_3km_spreadByEvent, MCS_MUCAPE_quart90ByEvent, MCS_wv_flux_quart90ByEvent, q_850_mean, MCS_MUCAPE_mean
x_label = 'spread in 0-3km bulk shear by event (kts)' # spread in 850hPa q by event (g/kg), spread in 0-3km bulk shear by event (kts)
...
if plot_all_growth_dist == True:
    ...
    plt.axvline(median_value, color='gray', linestyle='dashed', linewidth=2, label=f'Median: {median_value:.1f}')
...
if plot_rapid_growth_dist == True:
    ...
    plt.axvline(median_value, color='blue', linestyle='dashed', linewidth=2, label=f'Median: {median_value:.1f}')
...
if plot_slow_growth_dist == True:
    ...
    plt.axvline(median_value, color='red', linestyle='dashed', linewidth=2, label=f'Median: {median_value:.1f}')
...
plt.savefig(general_path + specific_outpath + '%s_hist_by_%s_%d%s%s%s%s%s.png' %(variable_name, var_seperation_name, seperation_threshold, dist_text_all_growth, dist_text_rapid_growth, dist_text_slow_growth, filter_label, events_removed_label), dpi=200)
