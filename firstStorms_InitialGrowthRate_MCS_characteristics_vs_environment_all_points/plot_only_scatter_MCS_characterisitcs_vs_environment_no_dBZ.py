...
var_plot_str = 'LFC' # Possible options: q_850, q_flux_850_v, srh_0_1km, srh_0_3km, srh_0_1km_noLatCorrection, srh_0_3km_noLatCorrection, MUCAPE, MUCIN, LCL, LFC, bulk_shear_0_3km_0, bulk_shear_0_6km_0, bulk_shear_2_6km, MLCAPE_100mb, MLCIN_100mb, rh_700, lapse_rate_3_8km, theta_e_diff_500_3000m

var_label = 'LFC - LCL (m)' # Possible options: '850mb q (g/kg)', srh_0_1km (m2/ms2), srh_0_3km (m2/ms2), 'bulk 1-6 km shear (kts)', 'MUCAPE (J/kg)', '3-8 km shear (kts)', r'$\Delta\theta_{e}$ 500-3000m (K)', '850mb v q flux (g $kg^{-1}$ m $s^{-1}$)'
...
max_refl_all_points_byEvent = pickle.load( open(general_path + specific_inpath + "%s_max_refl_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "rb"))
...
print('var_plot_all_points_byEvent.shape', var_plot_all_points_byEvent.shape)
print('len(var_plot_all_points_byEvent[0])', len(var_plot_all_points_byEvent[0]))
print('len(max_refl_all_points_byEvent[0])', len(max_refl_all_points_byEvent[0]))
print('len(var_plot_all_points_byEvent[1])', len(var_plot_all_points_byEvent[1]))
print('len(max_refl_all_points_byEvent[1])', len(max_refl_all_points_byEvent[1]))

# Creates a list of lists for all points where dBZ is equal to or less than 0 and keeps those values and sets those points for each event with a dBZ > 0 to NaN
var_plot_all_points_byEvent_masked = []
for v, r in zip(var_plot_all_points_byEvent, max_refl_all_points_byEvent):
    if v.shape != r.shape:
        ny = min(v.shape[0], r.shape[0])
        nx = min(v.shape[1], r.shape[1])
        v, r = v[:ny, :nx], r[:ny, :nx]
    var_plot_all_points_byEvent_masked.append(np.where(r <= 0, v, np.nan))

#var_plot_all_points_byEvent_masked = [np.where(r <= 0, v, np.nan)     # result is 2-D per event
#               for v, r in zip(var_plot_all_points_byEvent, max_refl_all_points_byEvent)]

#var_plot_all_points_byEvent_masked = np.where(max_refl_all_points_byEvent <= 0, var_plot_all_points_byEvent, np.nan)

print('len(var_plot_all_points_byEvent_masked[0])', len(var_plot_all_points_byEvent_masked[0]))
print('len(var_plot_all_points_byEvent_masked[1])', len(var_plot_all_points_byEvent_masked[1]))
...
if stat_type == 'mean':
    var_plot = np.array([np.nanmean(arr) for arr in var_plot_all_points_byEvent_masked])
elif stat_type == 'median':
    var_plot = np.array([np.nanmedian(arr) for arr in var_plot_all_points_byEvent_masked])
else:
...
#    ylabel = 'ccs growth rate (km2/hr)'
    ...
    for i, data in enumerate(data_list):
        ...
        # adding labels
        plt.ylabel('%s %s' %(stat_type, var_label))
        set_axis_style(ax, x_labels)
        
        # adding legend
        #plt.legend(*zip(*legend_labels), loc=2)
        
        filetype_label = '%s_%s-LCL_violin_by_%s%d' %(stat_type, var_plot_str, var_seperation_name, seperation_threshold)
...
plt.savefig(general_path + specific_outpath + '%s%s_wValueLabels_no_dBZ.png' %(filetype_label, flags), dpi=200)
