...
from wrf import (getvar, latlon_coords, interplevel, ll_to_xy, srhel, g_cape, cape_2d, cape_3d)
...
srh_0_1km = []
srh_0_3km = []
srh_0_1km_noLatCorrection = []
srh_0_3km_noLatCorrection = []
wv_flux_850_v = []
q_flux_850 = []
q_flux_850_v = []
rh_700 = []
...
#    # read in the variables 
    ...
    u10km = interplevel(u_original, hght_original, 10000)
    v10km = interplevel(v_original, hght_original, 10000)
    ...
    # calcualte shear at all points
    udiff_3_10 = u10km - u3km
    vdiff_3_10 = v10km - v3km
    
    shear3_10km = np.sqrt(udiff_3_10**2 +vdiff_3_10**2)

    bulk_shear_3_10km.append(np.array(shear3_10km))
    
    #print('shear3_10km', np.array(shear3_10km))
    
    ######### calculate storm relative helicity #########
    
    # read in the variables 
    hght_original = getvar(wrf_ncfile, 'height', msl=False, units='m')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])] # in m NOTE: this is AGL not MSL!!
    u_original_ms = getvar(wrf_ncfile, 'ua', units='m s-1')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])] # in kts
    v_original_ms = getvar(wrf_ncfile, 'va', units='m s-1')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])] # in kts
    
    # Get the terrain height
    terrain_original = getvar(wrf_ncfile, 'HGT')[int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]

    # Get the latitude and longitude points
    lats_original, lons_original = latlon_coords(terrain_original)
    
    srh_1000 = srhel(u_original_ms, v_original_ms, hght_original, terrain_original, top=1000.0, lats=lats_original)
    srh_3000 = srhel(u_original_ms, v_original_ms, hght_original, terrain_original, top=3000.0, lats=lats_original)
    
    srh_0_1km.append(np.array(srh_1000))
    srh_0_3km.append(np.array(srh_3000))
    
    srh_original_1000 = getvar(wrf_ncfile, 'helicity', top=1000.0)[int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    srh_original_3000 = getvar(wrf_ncfile, 'helicity', top=3000.0)[int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    
    srh_0_1km_noLatCorrection.append(np.array(srh_original_1000))
    srh_0_3km_noLatCorrection.append(np.array(srh_original_3000))
    ...
    ######### calculate MUCAPE, MUCIN, LCL, LFC #########
    
    # Extract necessary variables
    pres = getvar(wrf_ncfile, "pressure")[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    t = getvar(wrf_ncfile, "tk")[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]  # Temperature in Kelvin
    td = getvar(wrf_ncfile, "td")[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]  # Dewpoint temperature in Kelvin
    ...
    MCIN_original = getvar(wrf_ncfile, 'cape_2d')[1][int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    LCL_original = getvar(wrf_ncfile, 'cape_2d')[2][int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    LFC_original = getvar(wrf_ncfile, 'cape_2d')[3][int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    
    MUCAPE.append(np.array(MCAPE_original))
    print('median MCAPE', np.nanmedian(MCAPE_original))
    MUCIN.append(np.array(MCIN_original))
    print('median MCIN', np.nanmedian(MCIN_original))
    LCL.append(np.array(LCL_original))
    print('median LCL', np.nanmedian(LCL_original))
    LFC.append(np.array(LFC_original))
    print('median LFC', np.nanmedian(LFC_original))

    # get CAPE for parcels over the lowest 100mb
    CAPE_original = getvar(wrf_ncfile, 'cape_3d')[0][:9, int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    
    # take mean of parcels
    MLCAPE_100mb_original = np.nanmean(CAPE_original, axis=0)
    
    MLCAPE_100mb.append(MLCAPE_100mb_original)
    
    #print('MLCAPE_100mb_original', MLCAPE_100mb_original)
    
    CIN_original = getvar(wrf_ncfile, 'cape_3d')[1][:9, int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    
    # take mean of parcels
    MLCIN_100mb_original = np.nanmean(CIN_original, axis=0)
    
    MLCIN_100mb.append(MLCIN_100mb_original)
    
    #print('MLCIN_100mb_original', MLCIN_100mb_original)
    
    rh_original = getvar(wrf_ncfile, 'rh')[:,int(bottom_left_xy[1]):int(top_right_xy[1]),int(bottom_left_xy[0]):int(top_right_xy[0])]
    
    rh_700_original = interplevel(rh_original, pres, 700)
    
    rh_700.append(np.array(rh_700_original))
    
    #print('rh_700_original', np.array(rh_700_original))
    ...
    MCS_growth_stage_time_length_all_points_one_time = np.empty_like(q_subset_level)
    MCS_growth_stage_time_length_all_points_one_time[:] = MCS_growth_stage_time_length_filtered[count]
    MCS_growth_stage_time_length_all_points.append(MCS_growth_stage_time_length_all_points_one_time)
    
    print('MCS_growth_stage_time_length_all_points_one_time.shape', MCS_growth_stage_time_length_all_points_one_time.shape)
    print('MCS_growth_stage_time_length_all_points_one_time', MCS_growth_stage_time_length_all_points_one_time)
...
pickle.dump(MCS_growth_stage_time_length_all_points, open(general_outpath + specific_outpath + "%s_growth_stage_time_length_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(bulk_shear_3_10km, open(general_outpath + specific_outpath + "%s_bulk_shear_3_10km_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(srh_0_1km, open(general_outpath + specific_outpath + "%s_srh_0_1km_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(srh_0_3km, open(general_outpath + specific_outpath + "%s_srh_0_3km_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(srh_0_1km_noLatCorrection, open(general_outpath + specific_outpath + "%s_srh_0_1km_noLatCorrection_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(srh_0_3km_noLatCorrection, open(general_outpath + specific_outpath + "%s_srh_0_3km_noLatCorrection_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(MUCIN, open(general_outpath + specific_outpath + "%s_MUCIN_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(LCL, open(general_outpath + specific_outpath + "%s_LCL_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(LFC, open(general_outpath + specific_outpath + "%s_LFC_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(MLCAPE_100mb, open(general_outpath + specific_outpath + "%s_MLCAPE_100mb_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(MLCIN_100mb, open(general_outpath + specific_outpath + "%s_MLCIN_100mb_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(rh_700, open(general_outpath + specific_outpath + "%s_rh_700_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
MCS_growth_stage_time_length_all_points = np.concatenate([arr.flatten() for arr in MCS_growth_stage_time_length_all_points])
...
bulk_shear_3_10km = np.concatenate([arr.flatten() for arr in bulk_shear_3_10km])
q_850 = np.concatenate([arr.flatten() for arr in q_850])
srh_0_1km = np.concatenate([arr.flatten() for arr in srh_0_1km])
srh_0_3km = np.concatenate([arr.flatten() for arr in srh_0_3km])
srh_0_1km_noLatCorrection = np.concatenate([arr.flatten() for arr in srh_0_1km_noLatCorrection])
srh_0_3km_noLatCorrection = np.concatenate([arr.flatten() for arr in srh_0_3km_noLatCorrection])
MUCAPE = np.concatenate([arr.flatten() for arr in MUCAPE])
MUCIN = np.concatenate([arr.flatten() for arr in MUCIN])
MLCAPE_100mb = np.concatenate([arr.flatten() for arr in MLCAPE_100mb])
MLCIN_100mb = np.concatenate([arr.flatten() for arr in MLCIN_100mb])
LCL = np.concatenate([arr.flatten() for arr in LCL])
LFC = np.concatenate([arr.flatten() for arr in LFC])
...
rh_700 = np.concatenate([arr.flatten() for arr in rh_700])
...
print('MCS_growth_stage_time_length_all_points', MCS_growth_stage_time_length_all_points)
print('len(MCS_growth_stage_time_length_all_points)', len(MCS_growth_stage_time_length_all_points))
...
pickle.dump(MCS_growth_stage_time_length_all_points, open(general_outpath + specific_outpath + "%s_growth_stage_time_length_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(bulk_shear_3_10km, open(general_outpath + specific_outpath + "%s_bulk_shear_3_10km_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(srh_0_1km, open(general_outpath + specific_outpath + "%s_srh_0_1km_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(srh_0_3km, open(general_outpath + specific_outpath + "%s_srh_0_3km_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(srh_0_1km_noLatCorrection, open(general_outpath + specific_outpath + "%s_srh_0_1km_noLatCorrection_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(srh_0_3km_noLatCorrection, open(general_outpath + specific_outpath + "%s_srh_0_3km_noLatCorrection_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(MUCIN, open(general_outpath + specific_outpath + "%s_MUCIN_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(LCL, open(general_outpath + specific_outpath + "%s_LCL_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(LFC, open(general_outpath + specific_outpath + "%s_LFC_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(MLCAPE_100mb, open(general_outpath + specific_outpath + "%s_MLCAPE_100mb_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
pickle.dump(MLCIN_100mb, open(general_outpath + specific_outpath + "%s_MLCIN_100mb_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
...
pickle.dump(rh_700, open(general_outpath + specific_outpath + "%s_rh_700_all_points%s_%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "wb"))
