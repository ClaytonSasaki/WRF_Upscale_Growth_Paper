...
var1_plot_str = 'LCL' # Possible options: q_850, q_flux_850_v, srh_0_1km, srh_0_3km, srh_0_1km_noLatCorrection, srh_0_3km_noLatCorrection, MUCAPE, MUCIN, LCL, LFC, bulk_shear_0_3km_0, bulk_shear_0_6km_0, bulk_shear_2_6km, MLCAPE_100mb, MLCIN_100mb, rh_700, lapse_rate_3_8km, theta_e_diff_500_3000m

var2_plot_str = 'q_850'

var1_label = 'LCL (m)' # Possible options: '850mb q (g/kg)', srh_0_1km (m2/ms2), srh_0_3km (m2/ms2), 'bulk 1-6 km shear (kts)', 'MUCAPE (J/kg)', '3-8 km shear (kts)', r'$\Delta\theta_{e}$ 500-3000m (K)', '850mb v q flux (g $kg^{-1}$ m $s^{-1}$)'

var2_label = '850mb q (g/kg)'
...
var1_plot_all_points_byEvent = pickle.load( open(general_path + specific_inpath + "%s_%s_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, var1_plot_str, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "rb"))
var2_plot_all_points_byEvent = pickle.load( open(general_path + specific_inpath + "%s_%s_all_points_byEvent%s_%s_%s_%s.dat" %(MCS_file_label, var2_plot_str, filter_label, MCS_init_area, SALLJ_search_area, env_search_area), "rb"))
...
if events_removed == True:
    ...
    events_bool = np.full(len(var1_plot_all_points_byEvent), True)
...
var1_plot_all_points_byEvent = np.array(var1_plot_all_points_byEvent, dtype="object")[events_bool]
var2_plot_all_points_byEvent = np.array(var2_plot_all_points_byEvent, dtype="object")[events_bool]
...
if stat_type == 'mean':
    var1_plot = np.array([np.nanmean(arr) for arr in var1_plot_all_points_byEvent])
    var2_plot = np.array([np.nanmean(arr) for arr in var2_plot_all_points_byEvent])
elif stat_type == 'median':
    var1_plot = np.array([np.nanmedian(arr) for arr in var1_plot_all_points_byEvent])
    var2_plot = np.array([np.nanmedian(arr) for arr in var2_plot_all_points_byEvent])
else:
...
var1_plot = np.delete(var1_plot, nan_indices)
var2_plot = np.delete(var2_plot, nan_indices)

# remove events where chosen var is nan

nan_indices = np.isnan(var1_plot)
...
var1_plot_rapid_growth = var1_plot[rapid_growth]
var2_plot_rapid_growth = var2_plot[rapid_growth]
...
var1_plot_slow_growth = var1_plot[slow_growth]
var2_plot_slow_growth = var2_plot[slow_growth]
...
if plot_type == 'scatter':
    ...
    filename_x = var1_plot_str
    filename_y = var2_plot_str
    ...
    if add_SALLJ == True:
        ...
        ax.scatter(var1_plot_rapid_growth, var2_plot_rapid_growth, marker='o', color='blue', s=90)
        ax.scatter(var1_plot_slow_growth, var2_plot_slow_growth, marker='o', color='red', s=90)
...
plt.savefig(general_path + 'TESTTEST%s%s_wValueLabels.png' %(filetype_label, flags), dpi=200)
