...
Counts number of MCS events that have a SALLJ anytime during the growth period (first storms to MCS initation)
...
specific_inpath_2 = '%sarea_%s%s/data/' %(MCS_file_label, MCS_init_area, SALLJ_search_text)

MCS_prop_area_SALLJ_all_growth_rates = pickle.load(open(general_path_2 + specific_inpath_2 + "%s_prop_area_SALLJ_all_growth_times%s_%s_%s.dat" %(MCS_file_label, filter_label, MCS_init_area, SALLJ_search_area), "rb"))

print(MCS_prop_area_SALLJ_all_growth_rates.shape)

MCS_prop_area_SALLJ_max = np.nanmax(MCS_prop_area_SALLJ_all_growth_rates, axis=0)

MCS_prop_area_SALLJ_firstStorms = MCS_prop_area_SALLJ_all_growth_rates[0,:]

print(MCS_prop_area_SALLJ_max.shape)

# Filter events
if events_removed == True:

    events_bool = np.full(len(MCS_prop_area_SALLJ_max), True)
...
MCS_prop_area_SALLJ_max = np.array(MCS_prop_area_SALLJ_max)[events_bool]
MCS_prop_area_SALLJ_firstStorms = np.array(MCS_prop_area_SALLJ_firstStorms)[events_bool]

#print('MCS_prop_area_SALLJ_max', MCS_prop_area_SALLJ_max)
#print('MCS_prop_area_SALLJ_firstStorms', MCS_prop_area_SALLJ_firstStorms)
#print('diff', MCS_prop_area_SALLJ_max-MCS_prop_area_SALLJ_firstStorms)
#print('MCS_prop_area_SALLJ_max[MCS_prop_area_SALLJ_max>.2]', MCS_prop_area_SALLJ_max[MCS_prop_area_SALLJ_max>.2])
...
if seperation_plot == True:
    ...
    MCS_prop_area_SALLJ_max_mask = MCS_prop_area_SALLJ_max[mask]
    ...
    MCS_prop_area_SALLJ_max_mask = MCS_prop_area_SALLJ_max[:]
    
    var_seperation_label = ''

def count_SALLJ_at_growth_time(arr):
    """
    Checks each element in the array, 
    setting it to 1 if it meeting the 
    SALLJ coverage thresholds, 
    otherwise to 0.
    """
...
SALLJ_mask = count_SALLJ_at_growth_time(MCS_prop_area_SALLJ_max_mask)

print('# of MCS', len(MCS_center_lons_initiation_filtered_mask))

print('# of SALLJ', sum(SALLJ_mask))
