...
shear_type = '2-6km'               # Shear depth (e.g., '0-3km')
...
#    axs.scatter(MCS_center_lons_initiation_filtered_mask, MCS_center_lats_initiation_filtered_mask, transform=crs.PlateCarree(), color=cc_shear_dir, edgecolors=cc_area_SALLJ, linewidths=4.5, marker='*', zorder=7, s=1600)
    ...
    cc_shear_dir_SALLJ = [
        'green' if (shear_dir <= 45) or (shear_dir >= 300) else 'purple' if (135 <= shear_dir <= 241) else 'gray'
...
plt.savefig(general_outpath + '/larger_text_%s%s%s_initCentroids_%s_%s%s%s_241_300_differentMarkerStyles_circle_revision1.png' %(MCS_file_label, offset_label, var_seperation_label, shear_type, MCS_init_area, dir_shear_label, events_removed_label), dpi=600)
